...
public class VideoCallController {
    ...
    public ResponseEntity<RoomInfo> createRoom(@RequestBody CreateRoomRequest request,
        ...
        VideoRoom room = roomManager.createRoom(
                request.getRoomName(), userId, userName,
                request.getCallType(), request.getMaxParticipants()
        );
    ...
    public ResponseEntity<RoomInfo> getRoomDetails(@PathVariable String roomId) {
    ...
    public ResponseEntity<Map<String, Boolean>> isUserOnline(@PathVariable String userId) {
        return ResponseEntity.ok(Map.of("online", roomManager.isUserOnline(userId)));
    }
}
