...
public class VideoCallSignalHandler extends TextWebSocketHandler {
            ...
            if (room != null) {
                notifyRoom(
                        ...
                        SignalingMessage.builder()
                                .type(SignalingMessage.MessageType.USER_LEFT)
                                .roomId(roomId)
                                .senderId(userId)
                                .senderName(userName)
                                .build()
    ...
    private void handleCreateRoom(WebSocketSession session, SignalingMessage msg) throws IOException {
        ...
        VideoRoom room = roomManager.createRoom(
                ...
                msg.getSenderName(),
                msg.getCallType(),
                msg.getCallType() != null &&
                        msg.getCallType().equals("ONE_TO_ONE") ? 2 : 30
        );
        ...
        sendMessage(session,
                SignalingMessage.builder()
                        .type(SignalingMessage.MessageType.ROOM_CREATED)
    ...
    private void handleJoinRoom(WebSocketSession session,SignalingMessage msg) throws IOException {
        ...
        VideoRoom room = roomManager.joinRoom(
                msg.getRoomId(),
                msg.getSenderId(),
                msg.getSenderName(),
                msg.getRole(),
        ...
        if (room == null) {
            sendError(session,
                    "Room not found or full: " + msg.getRoomId());
            return;
        }
        ...
        notifyRoom(room,
                msg.getSenderId(),
                SignalingMessage.builder()
                        .type(SignalingMessage.MessageType.USER_JOINED)
        ...
        sendMessage(session,
                SignalingMessage.builder()
                        .type(SignalingMessage.MessageType.JOIN_ROOM)
                        .roomId(msg.getRoomId())
                        .senderId(msg.getSenderId())
    ...
    private void handleLeaveRoom(WebSocketSession session, SignalingMessage msg) throws IOException {
        ...
        VideoRoom room =
                roomManager.leaveRoom(msg.getSenderId(),
                        msg.getRoomId());
    ...
    private void handleCallUser(WebSocketSession session,  SignalingMessage msg) throws IOException {
        ...
        if (targetUserId == null ||
                !roomManager.isUserOnline(targetUserId)) {
        ...
        WebSocketSession targetSession =
                roomManager.getUserSession(targetUserId);

        if (targetSession != null && targetSession.isOpen()) {

            sendMessage(targetSession,
                    SignalingMessage.builder()
                            .type(SignalingMessage.MessageType.INCOMING_CALL)
                            .roomId(room.getRoomId())
                            .senderId(msg.getSenderId())
                            .senderName(msg.getSenderName())
                            .role(msg.getRole())
    ...
    private void handleCallAccepted(WebSocketSession session,  SignalingMessage msg) throws IOException {
        ...
        notifyRoom(room,
                msg.getSenderId(),
                SignalingMessage.builder()
                        .type(SignalingMessage.MessageType.CALL_ACCEPTED)
                        .roomId(msg.getRoomId())
                        .senderId(msg.getSenderId())
                        .senderName(msg.getSenderName())
                        .build());
    ...
    private void handleCallRejected(WebSocketSession session, SignalingMessage msg) throws IOException {
        ...
        VideoRoom room =
                roomManager.getRoom(msg.getRoomId());
        ...
        if (room != null) {
            ...
            notifyRoom(room,
                    msg.getSenderId(),
                    SignalingMessage.builder()
                            .type(SignalingMessage.MessageType.CALL_REJECTED)
                            .roomId(msg.getRoomId())
                            .senderId(msg.getSenderId())
                            .senderName(msg.getSenderName())
                            .build());
    ...
    private void notifyRoom(VideoRoom room,String excludeUserId,SignalingMessage message) {
        ...
        room.getSessions().forEach((id, session) -> {
    ...
    private void sendError(WebSocketSession session, String error)
        ...
        sendMessage(session,
                SignalingMessage.builder()
                        .type(SignalingMessage.MessageType.ERROR)
                        .payload(error)
                        .build());
    }
